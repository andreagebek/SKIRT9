...
#ifndef BPASSCHABRIER100SEDFAMILY_HPP
#define BPASSCHABRIER100SEDFAMILY_HPP
...
/** An instance of the BpassChabrier100SEDFamily class represents a BPASS family of single stellar population
    ...
    includes binary stellar systems and that assumes a Chabrier IMF with a lower mass limit of 1 solar mass
    and an upper mass limit of 100 solar masses (the "bin-imf_chab100" model).

    The SED templates are parametrized on metallicity (1e-5 - 0.04) and age (1 Myr - 100 Gyr), and
    scale with the initial mass of the SSP. By default, the wavelength grid has a resolution of 1 Angstrom over
    the complete range (1 Angstrom - 10 micron). This class provides a lower-resolution version which is also
    extrapolated up to 160 micron. This corresponds to a spectral resolution as shown in the figure below (tbd).
    ...
    download page on the SKIRT web site. (only if the high-resolution version is also kept!!!) */
class BpassChabrier100SEDFamily : public SEDFamily
{
    ITEM_CONCRETE(BpassChabrier100SEDFamily, SEDFamily, "a BPASS SED family for single stellar populations with an upper mass limit of 100 solar masses")
    ITEM_END()
...
public:
    /** This constructor can be invoked programmatically by classes that use a hard-coded SED
        ...
        */
    explicit BpassChabrier100SEDFamily(SimulationItem* parent);
